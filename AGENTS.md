...
1. Créer `_sass/components/_mon-composant.scss` avec la classe `.c-mon-composant`
2. Ajouter `@use 'mon-composant';` dans `_sass/components/_index.scss`
...
- Les cartes question sont pré-rendues via `_includes/apps/gluten-quiz-card.html` itéré par Liquid
...
1. Créer `apps/{nom}/index.html` avec front matter Jekyll (`layout: page`, `permalink`, etc.)
2. Créer `apps/{nom}/index.js` si JS nécessaire
3. Si des cards/fragments sont réutilisables : `_includes/apps/{nom}-card.html`
4. Si données YAML : `_data/{nom}.yml`
5. Si styles spécifiques : `_sass/components/_{nom}.scss` + import dans `_index.scss`
